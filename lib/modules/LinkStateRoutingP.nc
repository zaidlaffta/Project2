...
#define MAX_NODES 50  // Define maximum number of nodes
...
implementation {
    ...
    // Function to add routes to the local routing table
    void addRoute(uint16_t dest, uint16_t nextHop, uint16_t cost) {
        if (routeTableSize >= 10) {
            dbg(GENERAL_CHANNEL, "Routing table full, cannot add more routes.\n");
            return;
    ...
    // Command to print the global routing table
    command void LinkStateRouting.printGlobalRouteTable() {
        uint8_t i;
    ...
    // Command to print the local routing table
    command void LinkStateRouting.printRouteTable() {
        uint8_t i;
        ...
        if (routeTableSize == 0) {
            dbg(GENERAL_CHANNEL, "NO UPDATE FOR THE ROUTING TABLE, KEEPING DISCOVERY ON.\n");
            dbg(GENERAL_CHANNEL, "==============================\n");
            return;
        }
    ...
    void initDijkstra() {
        uint8_t i;
        for (i = 0; i < MAX_NODES; i++) {
            dist[i] = 0xFFFF;  // Set distance to infinity
            prev[i] = 0xFFFF;  // Set previous node to undefined
            visited[i] = FALSE; // Mark as unvisited
        }
        dist[TOS_NODE_ID] = 0; // Distance from self to self is 0
    }

    // Dijkstra's algorithm implementation
    command void LinkStateRouting.runDijkstra() {
        uint8_t i, j, u, minDist;
        uint16_t alt;  // Alternative distance
        ...
        for (i = 0; i < MAX_NODES; i++) {
            ...
            if (u == 0xFFFF) break;  // If no more reachable nodes, break

            visited[u] = TRUE;  // Mark node as visited

            // Update distances for neighbors in the routing table
            for (j = 0; j < routeTableSize; j++) {
                if (routeTable[j].dest == u) {
                    alt = dist[u] + routeTable[j].cost;  // Calculate alternative cost

                    if (alt < dist[routeTable[j].dest]) {
                        dist[routeTable[j].dest] = alt;
                        prev[routeTable[j].dest] = u;  // Update the previous node
                    }
                }
            }
        }
